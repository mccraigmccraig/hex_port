...
if Code.ensure_loaded?(Ecto) do
  defmodule DoubleDown.Repo do
    ...
    defcallback(
      delete_all(queryable :: Ecto.Queryable.t(), opts :: keyword()) ::
        {non_neg_integer(), nil | list()}
    )
    ...
    defcallback(
      get_by(queryable :: Ecto.Queryable.t(), clauses :: keyword() | map()) ::
        struct() | nil
    )
    ...
    """
    defcallback(
      get_by!(queryable :: Ecto.Queryable.t(), clauses :: keyword() | map()) :: struct(),
      bang: false
    )
    ...
    defcallback(
      aggregate(queryable :: Ecto.Queryable.t(), aggregate :: atom(), field :: atom()) ::
        term()
    )
    ...
    """
    defcallback(
      transact(fun_or_multi :: term(), opts :: keyword()) ::
        {:ok, term()} | {:error, term()},
      bang: false,
